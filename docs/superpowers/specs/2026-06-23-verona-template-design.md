# Verona template sobre @ts/ui — Diseño
...
usando la librería propia **@ts/ui** (HeroUI v3, React 19) + **@ts/tokens** en lugar de
...
| Gaps de @ts/ui (chart, calendar de eventos, rich-text, tree, timeline, file-upload) | Construir wrappers nuevos en @ts/ui **primero** (cada uno su mini-spec + TDD), no libs sueltas en la app. |
| Ubicación | App dentro del monorepo actual: `apps/verona`, consume `@ts/ui` y `@ts/tokens` vía `workspace:*`. |
...
| primeflex | Eliminado. Markup traducido a Tailwind v4 (ya provisto por `@ts/tokens/css`). |
...
- **Fase 1 — Gap wrappers en @ts/ui**. `chart`, `tree`, `rich-text`, `event-calendar`,
  `timeline`, `file-upload`. Cada uno: leer la lib externa, API propia, TDD, entry en exports map.
- **Fase 2 — Páginas que @ts/ui ya cubre**. auth (8), dashboards, uikit showcase (~18),
...
- `package.json`: `@ts/ui`, `@ts/tokens` como `workspace:*`; `lucide-react`, `next`, `react`,
...
- `app/layout.tsx` importa `@ts/tokens/css` (trae Tailwind v4 + HeroUI styles + tokens OKLCH).
...
- **CSS de layout**: portar `styles/layout/*.scss` de Verona (topbar, sidebar, breadcrumb,
  footer, content, config) **adaptado a vars de @ts/tokens**, NO reescrito a Tailwind puro.
...
Portados desde Verona, con PrimeReact reemplazado por @ts/ui. Cada componente es un client
component (`"use client"`) salvo los puramente estructurales.

| Componente | Origen Verona | Implementación @ts/ui |
|---|---|---|
...
| `AppTopbar` | `AppTopbar.tsx` | Logo, menubutton, tabs, search (`@ts/ui/search-field`), perfil (`@ts/ui/menu` + `@ts/ui/avatar`). |
...
| `AppBreadcrumb` | `AppBreadCrumb.tsx` | `@ts/ui/breadcrumbs`. |
| `AppFooter` | `AppFooter.tsx` | Estático. |
| `AppConfig` | `AppConfig.tsx` | `@ts/ui/drawer` + `@ts/ui/radio-group` + `@ts/ui/switch`/`button`. **Recortado a mínimo viable** (ver §5). |

### 4. Decisiones transversales

- **Iconos** — `apps/verona/lib/icon.tsx`: componente `<Icon name="pi pi-home" />` con un
  mapa `pi-*` → componente de `lucide-react`. Aísla el reemplazo; @ts/ui no se toca.
...
Fase 0 solo expone:
- **Color scheme**: light / dark → togglea `.dark` en `<html>` (clase que ya consume @ts/tokens).
...
- Los gap wrappers de @ts/ui (Fase 1).
