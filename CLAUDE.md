...
@lindaui/ui  (React 19, 78 entries)  ──wrap──►  @heroui/react@^3.2.1 (RAC)
       │ peer                                   ▲
...
pnpm release:flow            # automatiza push→CI/Release→merge del Version PR→verificación npm
pnpm release:flow --dry-run  # solo lee estado (branch/ahead-behind/PR abierto), no muta nada
```
...
**Familia chat (13 entries, gap-fill vs `@bienflow/ui-kit` — ver `docs/migration/from-bienflow-ui-kit.md`):** `chat-types` (tipos puros `GenericMessage`/`ConversationSummary`, sin runtime), `chat-layout` (shell responsive 3 columnas sidebar/main/detail, `activePanel` para mobile), `chat-header`, `chat-avatar` (bespoke con fallback a iniciales + `onError` — NO compone `@lindaui/ui/avatar`, que solo expone el Root de HeroUI sin `.Image`/`.Fallback`), `conversation-list`/`conversation-list-item` (búsqueda + filas, no filtra solo), `conversation-thread` (auto-scroll + agrupado por día vía `@lindaui/ui/time-grouping`), `message-bubble` (texto/imagen/audio/pdf + status), `message-composer` (Enter envía, Shift+Enter salto de línea), `audio-player` (nota de voz: play/pause + waveform estática), `chat-date-divider`/`chat-time-divider`, `empty-chat-view`. Sin `framer-motion` — animaciones CSS de `theme.css`. Más `time-range-picker` (presets + rango custom con `<input type="date">` nativo, sin `@internationalized/date`), `entity-health-card` (lista de salud genérica, ex-`GymHealthCard`), `fetching-indicator` (pill "Actualizando…").
...
**Entries no-componente de `@lindaui/ui` (hooks + util, 10 entries):** además de los 68 wrappers, `@lindaui/ui` exporta utilidades (cada `src/<n>.{ts,tsx}` no-test = una entry, igual que los componentes): `@lindaui/ui/search` (`normalizeText`/`matchesSearch`), `@lindaui/ui/cn` (re-export de `cn` de `tailwind-variants` — mismo merge+dedup tailwind que usa HeroUI; punto único, `chart.tsx` lo consume en vez de copia local), `@lindaui/ui/use-disclosure` (estado boolean para overlays controlados: `isOpen`/`open`/`close`/`toggle`, callbacks estables — HeroUI v3 no trae uno), `@lindaui/ui/use-media-query` (`useMediaQuery(q)` SSR-safe + helpers `useIsDesktop`/`useIsMobile`/`useIsTablet` ancladas a los breakpoints md 768px / [768,1024); el block `split-workspace` lo consume), `@lindaui/ui/use-debounce`, `@lindaui/ui/use-previous`, `@lindaui/ui/use-theme` (toggle light/dark con `localStorage` + `prefers-color-scheme`, togglea `.dark` en `<html>`), `@lindaui/ui/date` (`isToday`/`isYesterday`/`formatShortTime`/`formatRelativeTime` — `Intl`/`Date` nativos, sin `dayjs`), `@lindaui/ui/format` (`formatCurrency`/`formatDate`/`formatPhone` genérico) y `@lindaui/ui/time-grouping` (`groupByDay`, usado por `conversation-thread`). Los hooks llevan `"use client"`; test co-locado con `renderHook` (jsdom no trae `matchMedia`/`localStorage` de forma confiable en este entorno Node → los tests los stubean con un fake, el hook degrada a valores por default sin tirar).
...
1. **bunchee dts-bundle OOM** → el build es de dos pasos. El path de declaraciones bundleadas de bunchee revienta el heap de Node (>4GB) sobre las ~78 entries con el grafo de tipos pesado de HeroUI. `tsc --emitDeclarationOnly` es estable en memoria. **No volver a `bunchee` pelado.** `@types/react` + `@types/react-dom` son devDeps necesarias para que `tsc` emita (vitest no las necesitaba, tsc sí).
...
15. **Import relativo sin extensión que re-expone un TYPE de otro entry → attw `node16 (from ESM): Internal resolution error`.** Cuando un wrapper importa un valor de un entry hermano (`import { Button } from "./button"`) y ese valor NO aparece en su propia superficie de tipos pública, `tsc` no emite ningún `import` en el `.d.ts` — no hay problema (caso normal: `chart.tsx` usa `cn` de `./cn` solo en runtime). Pero si además importa un **type** y lo reexpone (`interface IconButtonProps extends Omit<ButtonProps, ...>`), `tsc` SÍ emite `import { type ButtonProps } from "./button"` en el `.d.ts`, preservando el specifier tal cual — sin extensión, porque `moduleResolution: "bundler"` no la exige. Node en ESM real sí la exige → el `.d.ts` queda con una import que no resuelve (bunchee/esbuild sí la resuelven bien en el `.js`, así que build+test+typecheck quedan en verde; solo lo detecta `attw --pack --profile esm-only`, que simula la resolución real de Node). Fix: en el `.tsx` que re-expone el type, escribir el import con `.js` (`from "./button.js"`) — `tsc`/esbuild bajo `moduleResolution: bundler` resuelven igual al `.tsx` real, y ahora el `.d.ts` emitido apunta a `dist/button.js`, que sí existe. Regla: **todo import relativo que reexporta/extiende un type de otro entry va con `.js`** (`type X } from "./y.js"`); los que solo consumen un valor en runtime pueden quedar sin extensión. Corrí `attw --pack --profile esm-only` (no solo build/test/typecheck) tras agregar un wrapper que compone otro (`icon-button`→`button`, los `chat-*` sobre `chat-types`) — es el único chequeo que lo pesca.
16. **`execFileSync("npm", ...)` en Windows tira `EINVAL` silencioso.** En Windows `npm` resuelve a un shim `npm.cmd`; `child_process.execFileSync`/`spawnSync` sin `shell: true` no lo puede spawnear (ni siquiera pidiendo `npm.cmd` explícito — sigue tirando `EINVAL`). Pisado en `scripts/release-flow.mjs` al verificar versiones publicadas: el `catch` silencioso hacía que TODO apareciera como "no publicado" aunque estuviera live en npm — un falso negativo, no un error visible. Fix: `execFileSync(process.platform === "win32" ? "npm.cmd" : "npm", args, { shell: process.platform === "win32" })`. Regla: cualquier script Node de `scripts/` que invoque `npm` (no `git`/`gh`, esos sí son binarios directos) necesita este tratamiento en Windows — y probar el resultado real (`console.log`), no solo que el `try/catch` no explote.

## Convenciones de tests

Vitest + jsdom + `@testing-library/react` + `user-event`. Co-locados. **Solo queries semánticas** (`getByRole`, `getByLabelText`, `findByRole`; `getByText` ok para bespoke sin rol) — sin `getByTestId`. Para componentes compound re-exportados que requieren `@internationalized/date` o composición pesada (date/color), el test asserta que el export resuelve (`toBeDefined`), no renderiza media composición. Para debuggear estructura RAC en jsdom: `console.log(container.innerHTML)`. **Cobertura: cero gap** — todo módulo fuente de `@lindaui/ui` aparece en ≥1 test (suite actual: 45 files / 154 tests en `ui`, 40 files / 167 tests en `blocks`). Dos suites compartidas agrupan los `toBeDefined`: `src/date-time-color.test.tsx` (date/time/color) y `src/compound-exports.test.tsx` (los 22 re-export: accordion/card/popover/tooltip/drawer/etc.). Trampas de test pisadas:
...
Changesets + Turborepo. `.github/workflows/ci.yml` corre el gate completo (build/test/typecheck/storybook/publint/attw) en PRs y en push a `main`. `.github/workflows/release.yml` corre en push a `main`: si hay changesets pendientes, `changesets/action` arma/actualiza el PR "chore: version packages" contra `changeset-release/main`; si no hay, publica directo con `pnpm -r publish --access public --no-git-checks` (NO `changeset publish`, ver Anti-regresión #13). `dist/` y `storybook-static/` están gitignored (se regeneran en CI). Bloqueos de publicación reales: ver "Autorizado / Prohibido".

**Automatización del babysitting (`scripts/release-flow.mjs` + skill `release-flow`):** push → watch CI+Release → si aparece el Version PR, watch su CI (auto-`gh run rerun` si cae en el flake de `action_required`) → merge → watch CI+Release post-merge → reporta versiones publicadas vía `npm view`. Sin pausa de confirmación a mitad de camino: invocar el skill/script YA es el pedido explícito que pide "Autorizado / Prohibido" antes de un publish — repetir la pregunta a mitad de flujo sería pedir el mismo permiso dos veces. Si cualquier corrida sale roja de verdad (no el flake), el script aborta sin mergear nada. `pnpm release:flow --dry-run` para validar el estado del repo sin mutar nada.
