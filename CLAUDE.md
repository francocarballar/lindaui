...
**Estado de publicación (importante):** **PUBLICADO.** Los 3 paquetes están live en npm bajo el scope `@lindaui` en `0.0.2` (`@lindaui/tokens`, `@lindaui/ui`, `@lindaui/blocks`). Org npm `@lindaui` creado, owner `francocarballar`. Metadata completa: `license: "MIT"` + `description` + `repository` + `publishConfig.access: "public"`; `@lindaui/ui`/`@lindaui/blocks` además `sideEffects: false` (tokens no, es CSS). `LICENSE` (MIT, `Copyright (c) 2026 Franco Carballar`) + `README.md` en raíz + los 3 paquetes, en el array `files` → entran al tarball. Git remote: `origin git@github.com:francocarballar/lindaui.git`. **El primer publish se hizo LOCAL** (`pnpm -r publish --access public --no-git-checks`) con un token granular **con bypass de 2FA** cargado vía `npm config set` (NO `echo`, que corrompe el .npmrc con BOM en PowerShell); changesets NO se usó por el bug de abajo. **CI de release (`release.yml`):** el step de publish es **`pnpm -r publish --access public --no-git-checks`** (NO `changeset publish`) — porque `changeset publish` (changesets@2.31 + npm 11) crashea en `isAlreadyPublishedError` con `TypeError: Cannot read properties of undefined (reading 'includes')` al parsear la respuesta de npm, enmascarando el error real. `pnpm -r publish` reemplaza `workspace:*`, saltea paquetes ya publicados, respeta `NPM_CONFIG_PROVENANCE`. El Version PR lo sigue armando changesets (el step solo cambia el comando de publish). **Único pendiente para que CI publique solo:** el secret `NPM_TOKEN` debe ser un token con **bypass 2FA** (Automation o granular-con-bypass) — un token sin bypass tira E403 que npm enmascara como E404 en el PUT. **Objetivo final — OIDC Trusted Publishing** (sin token): configurable AHORA que los paquetes existen → npmjs.com por paquete → Settings → Trusted Publisher → repo `francocarballar/lindaui`, workflow `release.yml`; después borrar el secret + la env `NPM_TOKEN`. El workflow ya tiene `id-token: write` + `NPM_CONFIG_PROVENANCE: true` (provenance sale solo desde CI, no del publish local). **El primer publish (0.0.2) se hizo LOCAL** (`pnpm -r publish`) con token bypass cargado vía `npm config set` (NO `echo`, corrompe el .npmrc con BOM en PowerShell).
