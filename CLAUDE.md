...
13. **CI de release (pnpm publish en GitHub Actions) — cadena de trampas pisadas.** El `release.yml` publica `@lindaui/*` con `pnpm -r publish` y costó hacerlo andar. En orden de aparición, NO repetir:
    ...
    - **Actions no crea PRs por default:** changesets arma el Version PR pero GitHub lo bloquea (`GitHub Actions is not permitted to create or approve pull requests`) salvo que actives Settings → Actions → General → Workflow permissions → "Allow GitHub Actions to create and approve pull requests". Atajo: FF-merge del branch `changeset-release/main` a `main` + push (equivale a mergear el Version PR → dispara el publish).
