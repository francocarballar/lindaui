...
**Estado de publicación (importante):** **PUBLICADO.** Los 3 paquetes están live en npm bajo el scope `@lindaui` en `0.0.2` (`@lindaui/tokens`, `@lindaui/ui`, `@lindaui/blocks`). Org npm `@lindaui` creado, owner `francocarballar`. Metadata completa: `license: "MIT"` + `description` + `repository` + `publishConfig.access: "public"`; `@lindaui/ui`/`@lindaui/blocks` además `sideEffects: false` (tokens no, es CSS). `LICENSE` (MIT, `Copyright (c) 2026 Franco Carballar`) + `README.md` en raíz + los 3 paquetes, en el array `files` → entran al tarball. Git remote: `origin git@github.com:francocarballar/lindaui.git`. **El primer publish se hizo LOCAL** (`pnpm -r publish --access public --no-git-checks`) con un token granular **con bypass de 2FA** cargado vía `npm config set` (NO `echo`, que corrompe el .npmrc con BOM en PowerShell); changesets NO se usó por el bug de abajo. **Pendiente para CI automático:** (1) el secret `NPM_TOKEN` debe ser un token con **bypass 2FA** (Automation o granular-con-bypass) — un token sin bypass tira E403 que npm enmascara como E404 en el PUT; (2) **bug `changeset publish` (changesets@2.31 + npm 11):** `isAlreadyPublishedError` crashea con `TypeError: Cannot read properties of undefined (reading 'includes')` al parsear la respuesta de npm → el publish de CI rompe aunque el auth esté ok. Workaround: cambiar el step de `release.yml` a `pnpm -r publish --access public --no-git-checks` (saltea changesets, reemplaza `workspace:*`), o subir `@changesets/cli`. (3) **OIDC Trusted Publishing** (objetivo final, sin token): configurable recién AHORA que los paquetes existen → npmjs.com por paquete → Settings → Trusted Publisher → repo `francocarballar/lindaui`, workflow `release.yml`; después borrar el secret + la env `NPM_TOKEN`. El workflow ya tiene `id-token: write` + `NPM_CONFIG_PROVENANCE: true` (provenance sale solo desde CI, no del publish local).
