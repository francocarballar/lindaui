# CLAUDE.md — @ts design system (@ts/tokens + @ts/ui sobre HeroUI v3)
...
2. La sección **"`@ts/ui` — estilos de wrapper"** antes de crear/editar cualquier componente.
...
|---|---|---|
| *"agregá el componente X"*, *"falta wrapper de Y"* | Componente nuevo en `@ts/ui` | Leer el `.d.ts` real de HeroUI + elegir estilo de wrapper (abajo) + TDD |
...
| Build de dos pasos, no se toca | `@ts/ui` build = `bunchee --no-dts && tsc -p tsconfig.build.json`. Razón en "Anti-regresiones #1". No volver a `bunchee` pelado. |
| Tokens via `@tailwindcss/cli` | Tailwind v4. Un `postcss` pelado NO procesa las directivas v4. Ver "build de @ts/tokens". |
...
apps/storybook  (Storybook 10 / react-vite — preview)
       │ consume ui + blocks
       ▼
@ts/blocks  (secciones compuestas)  ──compone──►  @ts/ui
@ts/ui  (React 19, 66 entries)  ──wrap──►  @heroui/react@^3.2.1 (RAC)
       │ peer                                   ▲
       │ depende (CSS en runtime)               │ estilos
       ▼                                         │
@ts/tokens  (dist/index.css)  ──@import──►  @heroui/styles  +  tailwindcss v4
```

**Direcciones (no negociables):**

- `@ts/ui` declara `react`, `react-dom`, `react-hook-form`, `@ts/tokens` como **peerDependencies** (no se instalan en el paquete publicado). `@heroui/react`, `tailwind-variants`, `tailwind-merge` son dependencies reales.
- `@ts/ui` **no** expone la composición RAC de HeroUI donde hay API custom.
- `@ts/blocks` compone primitivos de `@ts/ui` en secciones reusables (login-form, data-list, …). Declara `react`, `react-dom`, `react-hook-form`, `@ts/ui`, `@ts/tokens` como **peerDependencies**. Se distribuye como **paquete npm** (importar, NO source-copy / NO shadcn registry).
- `@ts/tokens` no tiene JS/React — es solo el pipeline CSS.
- `apps/storybook` consume `@ts/ui` + `@ts/blocks` via `workspace:*`; no se publica.

## Project Structure

| Componente | Package | Path |
|---|---|---|
| Tokens de marca (CSS) | `@ts/tokens` | `packages/tokens/` |
| Librería de componentes | `@ts/ui` | `packages/ui/` |
...
packages/tokens/
  ...
  dist/index.css         # generado (gitignored), export "@ts/tokens/css"
...
packages/blocks/         # secciones compuestas sobre @ts/ui (mismo build/test/exports que ui)
  src/<block>.tsx + <block>.test.tsx   # co-locados; "use client" si interactivo
  package.json           # exports DERIVADO (gen:exports); peers: @ts/ui, @ts/tokens, react-hook-form
...
apps/storybook/
  .storybook/{main.ts,preview.ts}   # preview importa "@ts/tokens/css"
...
cd packages/blocks && pnpm test                    # suite de blocks (requiere @ts/ui buildeado)
...
cd apps/storybook && pnpm gen:stories              # codegen de stories desde el exports de @ts/ui + @ts/blocks

# Release (DIFERIDO — ver Autorizado/Prohibido):
pnpm release                 # changeset publish (NO correr sin confirmar)
```

**No hay lint configurado.** El gate de tipos es `tsc -p tsconfig.build.json` (parte del build de `@ts/ui`).

## Build pipeline (por qué es de dos pasos)

`@ts/ui` y `@ts/blocks`: `bunchee --no-dts` genera el JS (esbuild, externaliza HeroUI + peers, preserva `"use client"`, resuelve extensiones de imports relativos) y `tsc -p tsconfig.build.json` genera los `.d.ts` (`emitDeclarationOnly`, excluye tests). Turbo ordena `@ts/tokens` → `@ts/ui` → `@ts/blocks` (por `^build`) y cachea. `@ts/blocks` externaliza `@ts/ui`, así que los tests/build de blocks necesitan el `dist/` de `@ts/ui` presente (turbo lo garantiza con `^build`).
...
**Entries no-componente de `@ts/ui` (hooks + util):** además de los 68 wrappers, `@ts/ui` exporta utilidades (cada `src/<n>.{ts,tsx}` no-test = una entry, igual que los componentes): `@ts/ui/search` (`normalizeText`/`matchesSearch`), `@ts/ui/cn` (re-export de `cn` de `tailwind-variants` — mismo merge+dedup tailwind que usa HeroUI; punto único, `chart.tsx` lo consume en vez de copia local), `@ts/ui/use-disclosure` (estado boolean para overlays controlados: `isOpen`/`open`/`close`/`toggle`, callbacks estables — HeroUI v3 no trae uno) y `@ts/ui/use-media-query` (`useMediaQuery(q)` SSR-safe + helpers `useIsDesktop`/`useIsMobile` ancladas al breakpoint md 768px; el block `split-workspace` lo consume). Los hooks llevan `"use client"`; test co-locado con `renderHook` (jsdom no trae `matchMedia` → los tests lo stubean, el hook degrada a `false` sin tirar).

## build de @ts/tokens

`scripts/build-css.mjs` escribe un `_entry.css` temporal encadenando `@import "tailwindcss"; @source "../ui/src"; @source "../blocks/src"; @import "@heroui/styles"; @import "./src/theme.css";` y lo corre por **`@tailwindcss/cli`** (Tailwind v4 — un `postcss` pelado no procesa las directivas v4). Los `@source` hacen que Tailwind escanee el source de `@ts/ui` + `@ts/blocks` y genere las **utilities** que la lib usa (flex/gap/max-w/bespoke como badge); sin ellos `index.css` trae HeroUI + tokens pero CERO utilities y los blocks/bespoke se ven sin estilo. Salida: `dist/index.css`, export `@ts/tokens/css` — un solo CSS que rinde la lib completa en cualquier consumidor. `_entry.css` está gitignored.
